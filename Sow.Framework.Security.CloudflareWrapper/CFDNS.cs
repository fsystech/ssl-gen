    ...
    public class CFDNS: ICFDNS {
        ...
        public async Task<ICFAPIResponse> RemoveRecord( IQueryConfig qConfig ) {
            try {
                ...
                _logger.Write( "Deleting DNS Record for {0}, Record type {1}", qConfig.DOMAIN_NAME, this.GetRecordText( qConfig.RECORD_TYPE ) );
                ...
                if ( !string.IsNullOrEmpty( resp.responseText ) ) {
                    _logger.Write( $"RemoveRecord-> {qConfig.DOMAIN_NAME} {this.GetRecordText( qConfig.RECORD_TYPE )}" );
