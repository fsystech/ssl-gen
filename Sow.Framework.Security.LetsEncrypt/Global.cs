    ...
    sealed class Global {
        ...
        public static void Load( string physicalPath ) {
            ...
            try {
                ...
                gConfig.config.ForEach( a => {
                    a.Domain.ForEach( d => {
                        if ( d.appSettings.WebServerName == "NGINX" ) {
                            d.appSettings.webServerEnum = WebServerEnum.NGINX;
                        } else if ( d.appSettings.WebServerName == "IIS" ) {
                            d.appSettings.webServerEnum = WebServerEnum.IIS;
                        } else {
                            d.appSettings.webServerEnum = WebServerEnum.NONE;
                        }
                    } );
                } );
            } catch (Exception e) {
